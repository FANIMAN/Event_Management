<template>
  <div class="bg-gray-200 min-h-screen flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-md w-full md:w-3/5"> <!-- Adjusted width to 4/5 of the screen -->
        ...
        <div class="flex flex-col gap-6 w-full md:w-1/2 p-6">
          ...
          <div class="flex flex-col gap-4">
            <div class="flex flex-row gap-6 md:gap-10 text-[18px]">
              <button @click="toggleContent('aboutEvent')"
                :class="{ 'text-[#D3AF35] underline text-[16px] md:text-[20px]': activeTab === 'aboutEvent', 'text-[#818181] text-[16px] md:text-[20px] hover:underline focus:outline-none': activeTab !== 'aboutEvent' }">About
                Event</button>
              <button @click="toggleContent('eventLocation')"
                :class="{ 'text-[#D3AF35] underline text-[16px] md:text-[20px]': activeTab === 'eventLocation', 'text-[#818181] text-[16px] md:text-[20px] hover:underline focus:outline-none': activeTab !== 'eventLocation' }">Event
                Location</button>
              <button @click="toggleContent('contact')"
                :class="{ 'text-[#D3AF35] underline text-[16px] md:text-[20px]': activeTab === 'contact', 'text-[#818181] text-[16px] md:text-[20px] hover:underline focus:outline-none': activeTab !== 'contact' }">Contact</button>
            </div>
          ...
          <div class="flex flex-row  justify-between text-[12px] md:text-lg">
            ...
            <div class="flex flex-row gap-2 items-center ">
              ...
              <p class="text-[#171625] font-bold">200 ETB /ticket</p>
            </div>
        ...
        <div class="flex flex-col justify-between w-full md:w-1/2 px-6 md:p-6">
          ...
          <div class="flex flex-col gap-6 md:gap-10">
            ...
            <div class="relative">
              ...
              <div class="flex items-center border border-gray-300 rounded-md">
                <button @click="decrement"
                  class="w-10 h-full py-1 ml-1 flex items-center justify-center bg-[#D3AF35] text-white font-bold rounded-md cursor-pointer focus:outline-none">-</button>
                <input type="number" id="numberOfTickets" v-model="numberOfTickets"
                  class="w-full px-4 py-2 text-center flex-1" min="0">

                <button @click="increment"
                  class="w-10 h-full py-1 mr-1 flex items-center justify-center bg-[#D3AF35] text-white font-bold rounded-md cursor-pointer focus:outline-none">+</button>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-2">
              ...
              <!-- <p>{{ getImage(banksAndMobiles[1].image) }}</p> -->
              <!-- <img :src="getImage(banksAndMobiles[1].image)"> -->
              <!-- <img :src="`@/static/${banksAndMobiles[1].image}`" /> -->
              <!-- <img :src="require(`@/static/${banksAndMobiles[1].image}`)" /> -->

              <!-- <p>{{ `~/static/${banksAndMobiles[0].image}` }}</p> -->




              <div class="grid grid-cols-3 gap-5 md:flex flex-row">
                <button v-for="(method, index) in banksAndMobiles" :key="index"
                  class="w-[50px] h-[50px] md:w-[60px] md:h-[60px] rounded-xl">
                  <!-- <img class="cursor-pointer hover:scale-105 rounded-md md:h-[60px] md:w-[60px]"
                    :src="require(`@/static/${method.image}`)" alt="image" /> -->

                  <img class="cursor-pointer hover:scale-105 rounded-md md:h-[60px] md:w-[60px]"
                    :src="`~/static/${method.image}`" alt="image" />
          ...
          <!-- Pay now Button -->
          <button class="bg-[#D3AF35] hover:bg-[#D3AF39] text-white font-bold py-3 px-4 text-xl rounded">Pay now</button>
        </div>
      </div>
    </div>
  </div>
...
// Define getImagePath function to get the image path dynamically
const getImagePath = (imageName) => {
  return require(`~/assets/img/${imageName}`);
};
const getImage = async imgName => {
  // set the relative path to assets
  const module = await import(`../assets/${imgName}`)
...
<div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center z-50">
  ...
  <div class="relative bg-white rounded-lg shadow-md w-full md:w-3/5 p-6 py-32 overflow-y-auto max-h-full">
    ...
    <div class="flex flex-col md:flex-row md:gap-6 space-y-4 md:space-y-0 md:space-x-4">
      <div class="flex flex-col gap-10 w-full md:w-1/2 p-6 items-center text-center">
        ...
        <p class="text-[#171625] text-[32px] font-bold">Event has been booked successfully!</p>
